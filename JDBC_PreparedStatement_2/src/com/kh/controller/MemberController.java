...
public class MemberController {
	...
	public void insertMember(String userId,String userPwd,String userName,String gender,String age,String email,String phone,String address,String hobby) {
		...
		//방법1) 기본생성자호 생성한 후에 각 필드에 setter메소드를 통해서 일일이 담는 방법
		...
		if(result > 0) { //insert가 성공적으로 완료되었구나
			...
			new MemberMenu().displayFail("회원 추가를 실패햐였습니다");
		}
	...
	public void selectList() {
		...
		if(list.isEmpty()) { //list가 비어 있을 경우
			new MemberMenu().dispalyNoData("전체조회 결과가 없습니다");
	...
	/**
	 * 사용자의 아이디로 회원 검색 요청을 처리해주는 매서드
	 * @param userId : 사용자가 검색하고자하는 회원 아이디
	 */
	public void selectByUserId(String userId) {
		Member m = new MemberDao().selectByUserId(userId);
		
		if(m == null) {
			new MemberMenu().dispalyNoData(userId+"해당 결과가 없습니다.");
		}else {
	 ...
	 * @param keyword : 사용자가 입력한 검색할 키워드 명
	 */
	public void selectByUserName(String keyword) {
		...
		if(list.size()>0) {//리스트에 검색결과가 담겨 있을 때
			...
			new MemberMenu().dispalyNoData("결과 없음");
		}
	}
	...
	public void updateMember(String userId,String userPwd,String email,String phone,String address) {
		...
		if(result > 0) {
			new MemberMenu().displaySuccess("성공적으로 회원정보 변경");
		}else {
	...
	public void deleteMember(String userId) {
		...
		if(result > 0) {
			new MemberMenu().displaySuccess("성공적으로 회원탈퇴처리");
		}else {
