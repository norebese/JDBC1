...
public class RunTest {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * *JDBX 과정(순서 중요) 1) jdbc driver 등록 : 해당 DBMS(오라클)가 제공하는 클래스 등록 2) Connection
		 * 생성 : 연결하고자 하는 DB정보를 입력해서 해당 DB와 연결하면서 생성 3) Statement 생성 : Connection 객체를
		 * 이용해서 생성한다(sql문 실행 및 결과받는 객체) 4) sql문 전달하면서 실행: Statement 객체를 이용해서 sql문 실행 5)
		 * 결과받기 > SELECT문 실행 => ResultSet 객체 (조회된 데이터들이 담겨있음) => 6_1_ > DML문 실행 =>
		 * int(처리된 행 수) =>6_2)
		 * 
		 * 6_1) ResultSet에 담겨있는 데이터들을 하나씩 하나씩 뽑아서 vo객체에 차근차근 옮겨담기[+ArrayList에 담아주기] 6_2)
		 * 트랜잭션 처리 (성공했다면 commit, 실패했다면 rollback 실행)
		 * 
		...
		// 1. 각자 pc(localhost)에 JDBC계정에 연결한 후 TEST데이블에 insert 해보기
...
//			System.out.println("OracleDriver 등록성공");
			...
			// 내가 실행 할 sql문이 dml문(insert, update, delete)일 경우 -> stmt.executeUpdate("dml문")
			// : int
			// 내가 실행 할 sql문이 select문일 경우 => stmt.executeUpdate("select문") : ResultSet
...
//					System.out.println("OracleDriver 등록성공");
			...
			//rset.next() => rset의 다음 행이 들어있는지 없는지 확인후 결과 반환 + 한 행을 내려준다
